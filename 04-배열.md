...
프로그래밍에서 데이터를 효율적으로 관리하고 처리하는 것은 소프트웨어 개발의 근간을 이루는 핵심 과제입니다. 지금까지 우리는 개별 변수를 사용하여 하나의 값을 저장하고 처리하는 방법을 배웠습니다. 하지만 실제 프로그램에서는 동일한 타입의 여러 데이터를 함께 다뤄야 하는 경우가 매우 빈번합니다. 예를 들어, 한 학급 30명 학생들의 성적을 저장하거나, 한 달 동안의 일별 온도 데이터를 관리하거나, 게임에서 여러 플레이어의 점수를 추적해야 할 때, 개별 변수로는 관리가 불가능하거나 극도로 비효율적입니다. `score1`, `score2`, `score3`... `score30`처럼 30개의 변수를 선언하는 것은 코드의 복잡성을 기하급수적으로 증가시키며, 유지보수를 거의 불가능하게 만듭니다.

이러한 문제를 해결하기 위해 프로그래밍 언어들은 **배열(Array)**이라는 자료구조를 제공합니다. 배열은 같은 타입의 여러 데이터를 하나의 이름으로 묶어서 순차적으로 저장하고 관리할 수 있는 컬렉션으로, 컴퓨터 과학에서 가장 기본적이면서도 중요한 선형 자료구조(Linear Data Structure)입니다. 배열을 사용하면 수십, 수백, 수천 개의 데이터를 효율적으로 저장하고 접근할 수 있으며, 반복문과 결합하여 강력한 데이터 처리 능력을 발휘합니다.

**배열의 역사적 배경:**

배열의 개념은 컴퓨터 과학의 여명기부터 존재했습니다. 1945년 존 폰 노이만(John von Neumann)이 제안한 프로그램 내장 방식(Stored-Program Concept)의 컴퓨터 아키텍처에서부터 메모리를 연속된 주소 공간으로 다루는 개념이 핵심이었으며, 이는 자연스럽게 배열의 개념으로 발전했습니다. 1957년 포트란(FORTRAN)에서 처음으로 배열이 프로그래밍 언어의 구문으로 도입된 이후, 배열은 거의 모든 프로그래밍 언어의 필수 요소가 되었습니다. C#의 배열은 이러한 전통을 계승하면서도 .NET Framework의 타입 안정성과 가비지 컬렉션을 통해 더욱 안전하고 편리하게 진화했습니다.
